...
function hui_get_thumbnail( $single=true, $must=true ) {  
    ...
    if ( has_post_thumbnail() ) {   
        ...
        foreach($images as $src){
            $html = sprintf('<span><img src="'. get_template_directory_uri() . '/images/thumbnail.png' .'" data-original="%s" class="thumb"/></span>', $src);
            break;
        }
...
function hui_load_scripts() {
    if (!is_admin()) {
        ...
        $jss = array(
            ...
            'baidu' => array(
                'jquery' => '//apps.bdimg.com/libs/jquery/2.0.0/jquery.min.js',
